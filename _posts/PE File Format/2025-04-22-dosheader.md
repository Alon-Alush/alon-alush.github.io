...
To correctly exit the program after executing our payload, we'll add the bytes `0x4C01` right after the `PE\0\0` signature. In 16-bit DOS, the opcode `4C` corresponds to the `INT 21h` function `4CH`, which terminates the current process with a return code. Following it, `01` is the exit code passed.
